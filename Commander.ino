...
char *folders[7] = {"GENERAL","FINDER","TEXT","SYSTEM","POWER", "TIPS", "SETTINGS"}; //max characters that fit in a line is 9 so the arrow fits
...
char *General[6][2] = {{"cmd + C", "Copy"}, {"cmd + V", "Paste"}, {"cmd + T", "New tab"}, {"cmd + N", "new window"}, {"cmd + W", "Close tab/window"}, {"cmd + shift + W", "Close all tabs/windows"}};
...
void loop() { //all of this isnt part of the actual code yet and will be changed
  ...
  for (int i = 0; i <= String(folder_descriptions[Select]).length(); i = i + 8) { //Loop by the amount of line needed to display the description within the space. i represents the chracter number so we increase by 8 becasue thats how many fit in a line
    ...
    if (String(String(folder_descriptions[Select]).substring(i, i + 8).charAt(0)) == " ") { //if the second line starts with a space then get rid of it and increase i by one to make up for the missing character.
      display.println(String(folder_descriptions[Select]).substring(i + 1, i + 9));
      i++;
    } else {
      display.println(String(folder_descriptions[Select]).substring(i, i + 8)); //print the line from the current character number and the next 8 characters to fill the space
    }
  }
...
void scroll_Items(String UpOrDown) {
...
void menu_Animation() {
  ...
  unsigned int box_box[][4] = {{36, 100, 36, 100}, {36, 96, 36, 103}, {36, 94, 36, 106}, {36, 85, 36, 114}, {36, 78, 36, 122}, {36, 75, 36, 125}, {36, 71, 36, 128}, {36, 71, 36, 128}, {36, 71, 36, 128}, {32, 71, 39, 128}, {29, 71, 42, 128}, {22, 71, 49, 128}, {17, 71, 58, 128}, {14, 71, 60, 128}, {11, 71, 63, 128}, {11, 71, 63, 128}, {11, 71, 63, 128}, {11, 71, 63, 128}, {11, 71, 63, 128}, {11, 71, 63, 128}};
  unsigned int box_box_line[7] = {1, 2, 4, 4, 4, 4, 4}; //helps the lines follow the box when it opens
  unsigned int box_box_openerY[10] = {33, 30, 23, 18, 15, 12, 12, 12, 12, 12}; //this list is here cause arduino couldnt add one succsefuflly in the command itself :(
  //cooridantes for the top bar
  unsigned int top_bar_line[8][2] = {{2, 75}, {10, 80}, {17, 87}, {34, 100}, {52, 114}, {61, 119}, {70, 119},  {70, 119}};
  unsigned int silly_array[9] = {3, 5, 10, 20, 35, 102, 118, 125, 125};
...
A working menu
    - Complete the UI with the animation (staitic is finished but without the animation)
        • Description box | FINISHED
          - Description Text | NOT FINISHED
        • Top Bar | STATIC
        • Text Handles | STATIC
        • Text | STATIC
        • Selection pointer | NOT FINISHED
    ...
    - navigation with the four buttons (up and down do exactly what they say. right and 
      left go in and out folders respectively)
